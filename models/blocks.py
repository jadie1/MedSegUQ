...
from models.layers import ConcreteDropout3D
from models.layers import BE_Conv3d, BE_ConvTranspose3d
...
"""
Adapted from https://github.com/Project-MONAI/MONAI/blob/dev/monai/networks/blocks/acti_norm.py
Added option for concrete dropout
"""
class ADN(nn.Sequential):
    ...
    def __init__(
        ...
        concrete_dropout_input_layer: nn.Module | None = None,
    ) -> None:
        super().__init__()
        ...
        if dropout is not None:
            if norm_dim is None and dropout_dim is None:
                raise ValueError("norm_dim or dropout_dim needs to be specified.")
            if dropout == 'concrete_dropout':
                op_dict["D"] = ConcreteDropout3D(concrete_dropout_input_layer) # TODO select concrete dropout module based on dropout_dim
            else:
...
Added option for concrete dropout and batch ensemble
"""
class Convolution(nn.Sequential):
    ...
    def __init__(
        ...
        if batch_ensemble:
            # TODO pass num_members 
            conv_type =  BE_ConvTranspose3d if is_transposed else BE_Conv3d
        elif lp_bnn:
            conv_type = LP_BNN_ConvTranspose3d if is_transposed else LP_BNN_Conv3d
        elif rank1_bnn:
            conv_type = Rank1_BNN_ConvTranspose3d if is_transposed else Rank1_BNN_Conv3d
        else:
        ...
        if concrete_dropout:
            self.add_module(
                ...
                ADN(
                    ...
                    dropout='concrete_dropout',
                    concrete_dropout_input_layer=conv,
                ),
            )
        else:
            self.add_module("conv", conv)
